...
public class TileSpring : Tile {
	...
	public override void trigger(Collider2D other){
		...
		if(currentMovement == Dillo.DILO_DOWN){
			newPos = new Vector3(transform.position.x,transform.position.y - 2*Level.TILESIZE,transform.position.z-2);
		}else if(currentMovement == Dillo.DILO_UP){
			newPos = new Vector3(transform.position.x,transform.position.y + 2* Level.TILESIZE,transform.position.z+2);
		}else if(currentMovement == Dillo.DILO_LEFT){
			newPos = new Vector3(transform.position.x - 2*Level.TILESIZE ,transform.position.y,transform.position.z);
